...
public class KeyPairServiceFactoryTest extends BaseTest {
  {
    Spectrum.describe("Keypair Service Factory", () -> {
      Spectrum.it(" returns same instance for configuration", () -> {
        ...
        KeyPairServiceConfiguration config = KeyPairServiceConfiguration.configure()
            .cipherAlgorithm("Blowfish").secretKeySpec("PBKDF2WithHmacSHA1").compile();
    ...
    Spectrum.describe("Mnemonic keypair generation tests", () -> {
      ...
      HDWallet restoredKeyPairWithSamePWD = keyPairService.recoverMasterMnemonicKeyPair(
      ...
      Spectrum.it("mnemonic keypair recovered from word seed list is same", () -> {
        Assert.assertEquals(Hex.toHexString(generatedKeyPair.getMasterKeyPair().getRawPrivateKey()),
