...
public class KeyPairServiceFactoryTest extends BaseTest {
	{
		Spectrum.describe("Keypair Service Factory", () -> {
			Spectrum.it(" returns same instance for configuration", () -> {
				KeyPairServiceFactory keypairServiceFactory = new KeyPairServiceFactory();

				KeyPairServiceConfiguration config = KeyPairServiceConfiguration.configure().cipherAlgorithm("Blowfish")
						.secretKeySpec("PBKDF2WithHmacSHA1").compile();
		...
		Spectrum.describe("Mnemonic keypair generation tests", () -> {
			...
			MnemonicKeyPair generatedKeyPair = keyPairService.generateMnemonicKeyPair(mnemonicSeedPassword);
			MnemonicKeyPair restoredKeyPairWithSamePWD = keyPairService
					.recoverMnemonicKeyPair(generatedKeyPair.getMnemonicSeedWords(), mnemonicSeedPassword);
			MnemonicKeyPair restoredKeyPairWithoutPWD = keyPairService
					.recoverMnemonicKeyPair(generatedKeyPair.getMnemonicSeedWords(), null);
