...
public class TransactionContractsTest extends BaseTest {
	...
	public void buildCreateContractTransactionTest(TestContext context) {
		this.executeTest(context, t -> {
			...
			ContractCreateTransactionModel contractTx = ContractCreateTransactionModel
					.builder().amount(amount)
					.callData(TestConstants.testContractCallData)
					.contractByteCode(TestConstants.testContractByteCode)
					.deposit(deposit).fee(fee).gas(gas).gasPrice(gasPrice)
					.nonce(getNextBaseKeypairNonce()).ownerId(ownerId).ttl(ttl)
			...
			_logger.debug("Call contract tx hash (debug unsigned): "
					+ unsignedTxDebug);
	...
	public void buildCallContractTransactionTest(TestContext context) {
		this.executeTest(context, t -> {
			...
			ContractCallTransactionModel callTx = ContractCallTransactionModel
					.builder().callData(callContractCalldata)
			...
			_logger.info("Call contract tx hash (native unsigned): "
					+ unsignedTxNative);

			String unsignedTxDebug = this.aeternityServiceDebug.transactions
					.blockingCreateUnsignedTransaction(callTx);

			_logger.info("Call contract tx hash (debug unsigned): "
					+ unsignedTxDebug);
	...
	public void staticCallContractOnLocalNode(TestContext context) {
		this.executeTest(context, t -> {
			...
			String calldata = encodeCalldata(
			...
			DryRunTransactionResults results = this.aeternityServiceNative.transactions
					.blockingDryRunTransactions(DryRunRequest.builder().build()
							...
							.account(DryRunAccountModel.builder()
									.publicKey(baseKeyPair.getPublicKey())
									.build())
							.transactionInputItem(createUnsignedContractCallTx(
									context, nonce, calldata, null))
							.transactionInputItem(createUnsignedContractCallTx(
									context, nonce.add(ONE), calldata, null)));
		...
		this.executeTest(context, t -> {
			...
			DryRunTransactionResults results = this.aeternityServiceNative.transactions
					.blockingDryRunTransactions(DryRunRequest.builder().build()
							.account(DryRunAccountModel.builder()
									...
									.build())
							.transactionInputItem(createUnsignedContractCallTx(
									context, getNextBaseKeypairNonce(),
									calldata, null)));
			...
			for (DryRunTransactionResult result : results.getResults()) {
				...
				ContractCallTransactionModel callTx = ContractCallTransactionModel
						...
						.callerId(baseKeyPair.getPublicKey()).ttl(ZERO)
						.virtualMachine(targetVM).build();

				PostTransactionResult response = this.aeternityServiceNative.transactions
						.blockingPostTransaction(callTx);

				context.assertEquals(response.getTxHash(),
						this.aeternityServiceNative.transactions
								.computeTxHash(callTx));
				...
				try {
					TransactionInfoResult txInfoObject = waitForTxInfoObject(
							response.getTxHash());
					Object decodedValue = decodeCallResult(
							TestConstants.testContractSourceCode,
							TestConstants.testContractFunction,
	...
	private String createUnsignedContractCallTx(TestContext context,
	...
	private ContractCallTransactionModel createCallContractModel(
		...
		ContractCallTransactionModel model = ContractCallTransactionModel
				...
				.nonce(nonce).callerId(callerId).ttl(ttl)
				.virtualMachine(targetVM).build();
		return model;
	}
	...
	public void callContractOnLocalNodeTest(TestContext context)
		...
		this.executeTest(context, t -> {
			...
			PostTransactionResult result = this.aeternityServiceNative.transactions
					.blockingPostTransaction(createCallContractModel(
							getNextBaseKeypairNonce(), callData, null));
			context.assertEquals(result.getTxHash(),
					this.aeternityServiceNative.transactions.computeTxHash(
							createCallContractModel(getNextBaseKeypairNonce(),
									callData, null)));
	...
	public void deployContractNativeOnTestNetworkTest(TestContext context)
		...
		ContractCreateTransactionModel testnetCreateTx = ContractCreateTransactionModel
				.builder().amount(amount)
				.callData(TestConstants.testContractCallData)
				.contractByteCode(TestConstants.testContractByteCode)
				.deposit(deposit).gas(gas).gasPrice(gasPrice).nonce(nonce)
				.ownerId(ownerId).ttl(ttl).virtualMachine(targetVM).build();

		PostTransactionResult result = testnetService.transactions
				.blockingPostTransaction(testnetCreateTx);
