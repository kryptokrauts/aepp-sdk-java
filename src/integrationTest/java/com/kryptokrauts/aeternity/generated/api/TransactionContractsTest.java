...
public class TransactionContractsTest extends BaseTest {
	...
	public void staticCallContractOnLocalNode(TestContext context) {
		...
		DryRunTransactionResults results = this.aeternityServiceNative.transactions
				.blockingDryRunTransactions(DryRunRequest.builder().build()
						...
						.transaction(createUnsignedContractCallTx(context, nonce.add(ONE), calldata, null)));
	...
	public void callContractAfterDryRunOnLocalNode(TestContext context) throws Throwable {
		...
		DryRunTransactionResults results = this.aeternityServiceNative.transactions
				.blockingDryRunTransactions(DryRunRequest.builder().build()
						.account(DryRunAccountModel.builder().publicKey(baseKeyPair.getPublicKey()).build())
						.transaction(createUnsignedContractCallTx(context, getNextBaseKeypairNonce(), calldata, null)));
		...
		for (DryRunTransactionResult result : results.getResults()) {
			...
			ContractCallTransactionModel callTx = ContractCallTransactionModel.builder().abiVersion(ONE)
					.callData(calldata).contractId(localDeployedContractId)
					.gas(result.getContractCallObject().getGasUsed())
					.gasPrice(result.getContractCallObject().getGasPrice()).nonce(getNextBaseKeypairNonce())
					.callerId(baseKeyPair.getPublicKey()).ttl(ZERO).build();
	...
	public void aDeployContractNativeOnLocalNode(TestContext context) throws Throwable {
		...
		ContractCreateTransactionModel contractTx = ContractCreateTransactionModel.builder().abiVersion(ONE)
				.amount(ZERO).callData(TestConstants.testContractCallData)
				.contractByteCode(TestConstants.testContractByteCode).deposit(ZERO).gas(gas).gasPrice(gasPrice)
				.nonce(getNextBaseKeypairNonce()).ownerId(baseKeyPair.getPublicKey()).ttl(ZERO).vmVersion(vmVersion)
				.build();
	...
	public void callContractOnLocalNodeTest(TestContext context) throws Throwable {
		...
		PostTransactionResult result = this.aeternityServiceNative.transactions
				.blockingPostTransaction(createCallContractModel(getNextBaseKeypairNonce(), callData, null));
		context.assertEquals(result.getTxHash(), this.aeternityServiceNative.transactions
				.computeTxHash(createCallContractModel(getNextBaseKeypairNonce(), callData, null)));
