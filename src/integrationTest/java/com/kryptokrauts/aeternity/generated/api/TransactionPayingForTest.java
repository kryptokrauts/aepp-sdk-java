...
public class TransactionPayingForTest extends BaseTest {
  ...
  public void testPayingForTx(TestContext testContext) {
    this.executeTest(testContext, t -> {
      ...
      SpendTransactionModel spendTx = SpendTransactionModel.builder().sender(account.getPublicKey())
          .recipient(delegationKeypair.getAddress()).amount(amount).nonce(nonce).build();
      aeternityServiceNative.transactions.blockingPostTransaction(spendTx);
      AccountResult delegationTestAccount =
          this.aeternityServiceNative.accounts.blockingGetAccount(delegationKeypair.getAddress());
      ...
      spendTx = SpendTransactionModel.builder().sender(delegationKeypair.getAddress())
          .recipient(anotherKeypair.getAddress()).amount(amount)
          .payload("yeah, somebody else payed the tx fee for this transaction =)")
          .nonce(delegationTestAccount.getNonce().add(ONE)).build();
      ...
      PayingForTransactionModel payingForTx =
          PayingForTransactionModel.builder().payerId(account.getPublicKey())
              .nonce(this.aeternityServiceNative.accounts.blockingGetAccount().getNonce().add(ONE))
              // .tx(signedInnerTx)
              .innerTxModel(spendTx)
              .privateKeyToSignerInnerTx(delegationKeypair.getEncodedPrivateKey()).build();
