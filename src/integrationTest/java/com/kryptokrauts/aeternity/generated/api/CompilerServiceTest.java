...
public class CompilerServiceTest extends BaseTest {
	...
	public void testCompileContract(TestContext context) {
		String byteCode = this.aeternityServiceNative.compiler.blockingCompile(TestConstants.testContractSourceCode,
				null, null);
	...
	public void testDecodeCalldata(TestContext context) {
		...
		Single<Object> callData = this.aeternityServiceNative.compiler
				.asyncDecodeCalldata(TestConstants.encodedServiceCallAnswer, "int");
		callData.subscribe(cd -> {
			_logger.info(cd.toString());
			context.assertEquals(new ObjectMapper().readValue(TestConstants.serviceCallAnswerJSON, Map.class)
					.get("value").toString(), JsonObject.mapFrom(cd).getInteger("value").toString());
			async.complete();
		}, throwable -> {
	...
	public void testGenerateACI(TestContext context) throws IOException {
		...
		ACIResult aci = this.aeternityServiceNative.compiler.blockingGenerateACI(paymentSplitterSource, null, null);
