...
public class CompilerServiceTest extends BaseTest {
	...
	public void testCompileContract(TestContext context) {
		this.executeTest(context, t -> {
			String byteCode = this.aeternityServiceNative.compiler
					.blockingCompile(TestConstants.testContractSourceCode, null,
							null)
					.getResult();
	...
	public void testDecodeCalldata(TestContext context) {
		this.executeTest(context, t -> {
			try {
				Object callData = this.aeternityServiceNative.compiler
						.blockingDecodeCalldata(
								TestConstants.encodedServiceCallAnswer, "int");
				_logger.info(callData.toString());
				context.assertEquals(
						new ObjectMapper()
								.readValue(TestConstants.serviceCallAnswerJSON,
										Map.class)
								.get("value").toString(),
						JsonObject.mapFrom(callData).getInteger("value")
								.toString());
			} catch (IOException e) {
				context.fail(e);
			}
	...
	public void testGenerateACI(TestContext context) throws IOException {
		this.executeTest(context, t -> {
			try {
				...
				String paymentSplitterSource = IOUtils.toString(inputStream,
						StandardCharsets.UTF_8.toString());
				ACIResult aci = this.aeternityServiceNative.compiler
						.blockingGenerateACI(paymentSplitterSource, null, null);
				_logger.info(aci.getEncodedAci().toString());
				context.assertEquals(TestConstants.paymentSplitterACI,
						aci.getEncodedAci().toString());
			} catch (IOException e) {
				context.fail(e);
			}
		});
