  ...
  public void testGaBlindAuthContract(TestContext context) {
    this.executeTest(context, t -> {
      ...
      SpendTransactionModel spendTx = SpendTransactionModel.builder().sender(account.getPublicKey())
          .recipient(gaAccountKeyPair.getAddress()).amount(amount).ttl(ZERO).nonce(nonce).build();
      ...
      String authData = encodeCalldata(TestConstants.testGABlindAuthContract, "authorize",
          Arrays.asList(String.valueOf(new Random().nextInt())), null);

      GeneralizedAccountsMetaTransactionModel gaMetaTx = GeneralizedAccountsMetaTransactionModel
          .builder().gaId(gaAccountKeyPair.getAddress()).authData(authData).virtualMachine(targetVM)
          // .tx(encodedInnerTx)
          .innerTxModel(gaInnerSpendTx).build();
