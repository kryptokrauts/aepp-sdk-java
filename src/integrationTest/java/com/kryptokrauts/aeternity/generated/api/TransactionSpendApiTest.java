...
public class TransactionSpendApiTest extends BaseTest {
	...
	public void buildNativeSpendTransactionTest(TestContext context) {
		...
		AbstractTransaction<?, SpendTransactionModel> spendTx = transactionServiceNative.getTransactionFactory()
				.createSpendTransaction();
		spendTx.getTxModel().setParameters().sender(sender).recipient(recipient).amount(amount).payload(payload)
				.ttl(ttl).nonce(nonce).wrap();
		UnsignedTx unsignedTxNative = transactionServiceNative.createUnsignedTransaction(spendTx).blockingGet();
...
//					.createSpendTransaction(keyPair.getPublicKey(), recipient, amount, payload, ttl, nonce);
//			UnsignedTx unsignedTxNative = transactionServiceNative.createUnsignedTransaction(spendTx).blockingGet();
...
//				context.assertEquals(it.getTxHash(), transactionServiceNative.computeTxHash(signedTx.getTx()));
//				async.complete();
//			}, throwable -> {
//				context.fail();
//			});
