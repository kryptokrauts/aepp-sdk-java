...
public class TransactionSpendApiTest extends BaseTest {
  ...
  public void buildNativeSpendTransactionTest(TestContext context) {
    this.executeTest(context, t -> {
      ...
      SpendTransactionModel spendTx = SpendTransactionModel.builder().sender(sender)
      ...
      String unsignedTxNative = aeternityServiceNative.transactions
          .asyncCreateUnsignedTransaction(spendTx).blockingGet().getResult();

      String unsignedTxDebug =
          aeternityServiceDebug.transactions.blockingCreateUnsignedTransaction(spendTx).getResult();
  ...
  public void postSpendSelfSignTxTest(TestContext context) {
    this.executeTest(context, t -> {
      ...
      SpendTransactionModel spendTx = SpendTransactionModel.builder().sender(account.getPublicKey())
          .recipient(recipient).amount(amount).payload(payload).ttl(ZERO).nonce(nonce).build();
  ...
  public void postSpendSignedTxString(TestContext context) {
    this.executeTest(context, t -> {
      ...
      String signedTxNative = aeternityServiceNative.transactions.signTransaction(unsignedTxNative,
          baseKeyPair.getEncodedPrivateKey());
    ...
    this.executeTest(context, t -> {
      try {
        ...
        SpendTransactionModel spendTx =
            SpendTransactionModel.builder().sender(this.baseKeyPair.getAddress())
                .recipient(recipient.getAddress()).amount(new BigInteger("1000000000000000000"))
                .payload("donation").ttl(ZERO).nonce(getNextBaseKeypairNonce()).build();
        PostTransactionResult txResponse =
            aeternityServiceNative.transactions.blockingPostTransaction(spendTx);
        _logger.info("SpendTx hash: " + txResponse.getTxHash());
        waitForTxMined(txResponse.getTxHash());
  ...
  public void postSpendTxWithModelAndPK(TestContext context) throws TransactionCreateException {
    this.executeTest(context, t -> {
      try {
        ...
        spendTx = SpendTransactionModel.builder().sender(recipient.getAddress())
            .recipient(baseKeyPair.getAddress())
            .amount(BigInteger.valueOf(recipientAccountSendAmount))
            .nonce(recipientAccount.getNonce().add(ONE)).ttl(ZERO).build();
        _logger.info("Sending back {}", spendTx);
        txResponse = aeternityServiceNative.transactions.blockingPostTransaction(spendTx,
            recipient.getEncodedPrivateKey());
        _logger.info("SpendTx hash: " + txResponse.getTxHash());
        waitForTxMined(txResponse.getTxHash());
        recipientAccount =
            this.aeternityServiceNative.accounts.blockingGetAccount(recipient.getAddress());
        context.assertEquals(recipientAccount.getBalance().longValue(),
            recipientAccountBalance - recipientAccountSendAmount - spendTx.getFee().longValue());
      } catch (Throwable e) {
        context.fail(e);
      }
    });
    ...
    this.executeTest(context, t -> {
      try {
        ...
        SpendTransactionModel spendTx = SpendTransactionModel.builder()
            .sender(this.baseKeyPair.getAddress()).recipient(recipient.getAddress())
            .amount(new BigInteger("1000000000000000000")).payload("wait for confirmation works =)")
            .ttl(ZERO).nonce(getNextBaseKeypairNonce()).build();
        Single<PostTransactionResult> postTransactionResultSingle =
            aeternityServiceNative.transactions.asyncPostTransaction(spendTx);
        postTransactionResultSingle.subscribe(postTransactionResult -> {
          _logger.info("SpendTx hash: " + postTransactionResult.getTxHash());
          Single<TransactionResult> transactionResultSingle = aeternityServiceNative.transactions
              .asyncWaitForConfirmation(postTransactionResult.getTxHash());
          transactionResultSingle.subscribe(transactionResult -> {
    ...
    this.executeTest(context, t -> {
      try {
        ...
        SpendTransactionModel spendTx =
            SpendTransactionModel.builder().sender(this.baseKeyPair.getAddress())
                .recipient(recipient.getAddress()).amount(new BigInteger("1000000000000000000"))
                .payload("wait for confirmation fails :-(").ttl(ZERO)
