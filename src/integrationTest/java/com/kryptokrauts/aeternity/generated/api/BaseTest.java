...
public abstract class BaseTest {
	...
	protected TransactionInfoResult waitForTxInfoObject(String txHash) throws Throwable {
		return callMethodAndGetResult(() -> aeternityServiceNative.info.asyncGetTransactionInfoByHash(txHash),
	...
	protected PostTransactionResult postTx(AbstractTransactionModel<?> tx) throws Throwable {
		...
		_logger.info(String.format("Transaction of type %s is mined at block %s with height %s", txValue.getTxType(),
				txValue.getBlockHash(), txValue.getBlockHeight()));
	...
	protected TransactionResult waitForTxMined(String txHash) throws Throwable {
		...
		while (blockHeight == -1 && doneTrials < TestConstants.NUM_TRIALS_DEFAULT) {
			minedTx = callMethodAndGetResult(() -> aeternityServiceNative.info.asyncGetTransactionByHash(txHash),
