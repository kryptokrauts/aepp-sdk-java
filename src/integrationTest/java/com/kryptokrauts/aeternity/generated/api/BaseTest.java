...
public abstract class BaseTest {
	...
	protected static final Logger _logger = LoggerFactory
			.getLogger("com.kryptokrauts.IntegrationTest");
	...
	public RunTestOnContext rule = new RunTestOnContext(
			new VertxOptions().setMaxWorkerExecuteTime(TEST_CASE_TIMEOUT_MILLIS)
	...
	public void setupApiClient(TestContext context)
		...
		baseKeyPair = keyPairService.generateBaseKeyPairFromSecret(
				TestConstants.BENEFICIARY_PRIVATE_KEY);

		aeternityServiceNative = new AeternityServiceFactory()
				.getService(AeternityServiceConfiguration.configure()
						...
						.network(Network.DEVNET).nativeMode(true)
						.baseKeyPair(baseKeyPair).vertx(vertx)
						.targetVM(targetVM).compile());
		aeternityServiceDebug = new AeternityServiceFactory()
				.getService(AeternityServiceConfiguration.configure()
						.baseUrl(getAeternityBaseUrl())
						.compilerBaseUrl(getCompilerBaseUrl())
						.aeternalBaseUrl(getAeternalBaseUrl())
						.network(Network.DEVNET).nativeMode(false)
						.baseKeyPair(baseKeyPair).vertx(vertx)
						.targetVM(targetVM).compile());
	}
	...
	protected static String getAeternalBaseUrl() throws ConfigurationException {
		...
		if (aeternalBaseUrl == null) {
			throw new ConfigurationException(
					"ENV variable missing: AETERNAL_BASE_URL");
		}
	...
	public static void startup() throws ConfigurationException {
		...
		_logger.info(String.format("%s: %s", AETERNITY_BASE_URL,
				getAeternityBaseUrl()));
		_logger.info(String.format("%s: %s", COMPILER_BASE_URL,
				getCompilerBaseUrl()));
		_logger.info(String.format("%s: %s", AETERNAL_BASE_URL,
				getAeternalBaseUrl()));
		_logger.info(
				"-----------------------------------------------------------------------------------");
	}
	...
	protected AccountResult getAccount(String publicKey) {
		return aeternityServiceNative.accounts
				.blockingGetAccount(Optional.ofNullable(publicKey));
	}

	protected TransactionInfoResult waitForTxInfoObject(String txHash)
			throws Throwable {
		return callMethodAndGetResult(
				() -> aeternityServiceNative.info
						.asyncGetTransactionInfoByHash(txHash),
				TransactionInfoResult.class);
	}

	protected PostTransactionResult blockingPostTx(
			AbstractTransactionModel<?> tx, Optional<String> privateKey)
			throws Throwable {
		PostTransactionResult postTxResponse = this.aeternityServiceNative.transactions
				.blockingPostTransaction(tx,
						privateKey.orElse(this.baseKeyPair.getPrivateKey()));
		...
		_logger.info(String.format(
				...
				txValue.getTxType(), txValue.getBlockHash(),
				txValue.getBlockHeight()));

		return postTxResponse;
	}

	protected PostTransactionResult postTx(AbstractTransactionModel<?> tx)
			throws Throwable {
		PostTransactionResult postTxResponse = callMethodAndGetResult(
				() -> this.aeternityServiceNative.transactions
						.asyncPostTransaction(tx),
	...
	protected void waitForBlockHeight(BigInteger blockHeight, Long timeoutMilli)
			throws Throwable {
		BigInteger currentBlockHeight = BigInteger.ZERO;
		_logger.info("waiting for blockHeight {} and checking every {} seconds",
				blockHeight, timeoutMilli / 1000d);
		while (currentBlockHeight.compareTo(blockHeight) == -1) {
			currentBlockHeight = aeternityServiceNative.info
					.blockingGetCurrentKeyBlock().getHeight();
			_logger.info("current blockHeight: {}", currentBlockHeight);
			Thread.sleep(timeoutMilli);
		}
	}

	protected String encodeCalldata(String contractSourceCode,
			String contractFunction, List<String> contractFunctionParams) {
		return this.aeternityServiceNative.compiler
				.blockingEncodeCalldata(contractSourceCode, contractFunction,
						contractFunctionParams)
				.getResult();
	}

	protected JsonObject decodeCalldata(String encodedValue,
			String sophiaReturnType) {
		return JsonObject.mapFrom(this.aeternityServiceNative.compiler
				.blockingDecodeCalldata(encodedValue, sophiaReturnType));
	}

	protected Object decodeCallResult(String source, String function,
			String callResult, String callValue) {
	...
	protected <T> T callMethodAndAwaitException(
			Supplier<Single<T>> observerMethod, Class<T> exception)
			throws Throwable {
		return callMethodAndGetResult(TestConstants.NUM_TRIALS_DEFAULT,
				observerMethod, exception, true);
	}

	protected <T> T callMethodAndGetResult(Supplier<Single<T>> observerMethod,
			Class<T> type) throws Throwable {
		return callMethodAndGetResult(TestConstants.NUM_TRIALS_DEFAULT,
				observerMethod, type, false);
	}

	protected <T> T callMethodAndGetResult(Integer numTrials,
			Supplier<Single<T>> observerMethod, Class<T> type,
		...
		do {
			...
			if (singleTestObserver.errorCount() > 0) {
				...
				if (doneTrials == numTrials) {
					_logger.error(
							"Following error(s) occured while waiting for result of call, aborting");
					for (Throwable error : singleTestObserver.errors()) {
						_logger.error(error.toString());
					}
					throw new InterruptedException(
							"Max number of function call trials exceeded, aborting");
				}
				_logger.warn(String.format(
