...
public abstract class BaseTest {
	...
	protected GenericSignedTx getTxByHash(String txHash, int numTrials)
		...
		int trialCounter = 0;
		do {
			Single<GenericSignedTx> genericSignedTxSingle = transactionServiceNative.getTransactionByHash(txHash);
	...
	protected void waitForTxInfoObject(TxInfoObject txInfoObject, String txHash) throws InterruptedException {
		boolean waiting = true;
		do {
			Single<TxInfoObject> txInfoObjectSingle = transactionServiceNative.getTransactionInfoByHash(txHash);
	...
	protected Calldata encodeCalldata(String contractSourceCode, String contractFunction,
		...
		Single<Calldata> callDataSingle = this.sophiaCompilerService.encodeCalldata(contractSourceCode,
				contractFunction, contractFunctionParams);
	...
	protected JsonObject decodeCalldata(String encodedValue, String sophiaReturnType) {
		...
		Single<SophiaJsonData> sophiaJsonDataSingle = this.sophiaCompilerService.decodeCalldata(encodedValue,
				sophiaReturnType);
	...
	protected DryRunResults performDryRunTransactions(List<Map<AccountParameter, Object>> accounts, BigInteger block,
		...
		Single<DryRunResults> dryRunResults = this.transactionServiceNative.dryRunTransactions(accounts, block,
				unsignedTxes);
	...
	protected UnsignedTx createUnsignedContractCallTx(String callerId, BigInteger nonce, String calldata,
			BigInteger gasPrice, String contractId, BigInteger amount) {
		...
		AbstractTransaction<?> contractTx = transactionServiceNative.getTransactionFactory()
				.createContractCallTransaction(abiVersion, calldata, contractId, gas,
		...
		Single<UnsignedTx> unsignedTxSingle = transactionServiceNative.createUnsignedTransaction(contractTx);
