	...
	public void postNameClaimTxTest(TestContext context) {
		...
		rule.vertx().executeBlocking(future -> {
			try {
				...
				UnsignedTx unsignedClaimTx = transactionServiceNative.createUnsignedTransaction(nameClaimTx)
						.blockingGet();
	...
	public void postUpdateTxTest(TestContext context) {
		...
		rule.vertx().executeBlocking(future -> {
			try {
				...
				UnsignedTx unsignedUpdateTx = transactionServiceNative.createUnsignedTransaction(nameUpdateTx)
						.blockingGet();
	...
	public void postRevokeTxTest(TestContext context) {
		...
		rule.vertx().executeBlocking(future -> {
			try {
				...
				UnsignedTx unsignedTx = transactionServiceNative.createUnsignedTransaction(nameRevokeTx).blockingGet();
				Tx signedTx = transactionServiceNative.signTransaction(unsignedTx, keyPair.getPrivateKey());
