    ...
    this.executeTest(context, t -> {
      ...
      NamePreclaimTransactionModel preclaim =
          NamePreclaimTransactionModel.builder().accountId(sender).name(validName).salt(salt)
              .nonce(getNextKeypairNonce()).ttl(ttl).build();
    ...
    this.executeTest(context, t -> {
      try {
        ...
        NamePreclaimTransactionModel namePreclaimTx =
            NamePreclaimTransactionModel.builder().accountId(keyPair.getAddress()).name(validName)
                .salt(salt).nonce(getNextKeypairNonce()).ttl(ZERO).build();
        ...
        NameClaimTransactionModel nameClaimTx =
            NameClaimTransactionModel.builder().accountId(keyPair.getAddress()).name(validName)
                .nameSalt(salt).nonce(getNextKeypairNonce()).ttl(ZERO).build();
  ...
  public void postUpdateAndSpendTxTest(TestContext context) {
    this.executeTest(context, t -> {
      try {
        ...
        NamePreclaimTransactionModel namePreclaimTx =
            NamePreclaimTransactionModel.builder().accountId(keyPair.getAddress()).name(name)
                .salt(salt).nonce(getNextKeypairNonce()).ttl(ZERO).build();
        ...
        NameClaimTransactionModel nameClaimTx =
            NameClaimTransactionModel.builder().accountId(keyPair.getAddress()).name(name)
                .nameSalt(salt).nonce(getNextKeypairNonce()).ttl(ZERO).build();
        ...
        NameUpdateTransactionModel nameUpdateTx =
            NameUpdateTransactionModel.builder().accountId(keyPair.getAddress())
                .nameId(nameEntryResult.getId()).nonce(getNextKeypairNonce()).ttl(ZERO)
        ...
        SpendTransactionModel spendTx = SpendTransactionModel.builder()
            .sender(this.keyPair.getAddress()).recipient(nameEntryResult.getId()).amount(aettos)
            .payload("send to AENS name test").ttl(ZERO).nonce(getNextKeypairNonce()).build();
    ...
    this.executeTest(context, t -> {
      try {
        ...
        NameRevokeTransactionModel nameRevokeTx =
            NameRevokeTransactionModel.builder().accountId(keyPair.getAddress()).nameId(nameId)
                .nonce(getNextKeypairNonce()).ttl(ZERO).build();
  ...
  public void zAuctionTest(TestContext context) {
    this.executeTest(context, t -> {
      try {
        ...
        /** create a new namespace to update later */
        NamePreclaimTransactionModel namePreclaimTx =
            NamePreclaimTransactionModel.builder().accountId(this.keyPair.getAddress()).name(name)
                .salt(salt).nonce(getNextKeypairNonce()).ttl(ZERO).build();
        PostTransactionResult namePreclaimResult = this.blockingPostTx(namePreclaimTx);
        _logger.info("NamePreclaimTx hash: {}", namePreclaimResult.getTxHash());
        context.assertEquals(namePreclaimResult.getTxHash(),
        ...
        NameClaimTransactionModel nameClaimTx =
            NameClaimTransactionModel.builder().accountId(this.keyPair.getAddress()).name(name)
                .nameSalt(salt).nonce(getNextKeypairNonce()).ttl(ZERO).build();
