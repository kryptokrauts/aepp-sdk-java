  ...
  public void buildNativeNamePreclaimTransactionTest(TestContext context) {
    this.executeTest(context, t -> {
      ...
      NamePreclaimTransactionModel preclaim =
          NamePreclaimTransactionModel.builder().accountId(sender).name(validName).salt(salt)
              .nonce(getNextBaseKeypairNonce()).ttl(ttl).build();

      String unsignedTxNative = this.aeternityServiceNative.transactions
          .blockingCreateUnsignedTransaction(preclaim).getResult();
    ...
    this.executeTest(context, t -> {
      try {
        ...
        NamePreclaimTransactionModel namePreclaimTx =
            NamePreclaimTransactionModel.builder().accountId(baseKeyPair.getAddress())
                .name(validName).salt(salt).nonce(getNextBaseKeypairNonce()).ttl(ZERO).build();
        ...
        NameClaimTransactionModel nameClaimTx =
            NameClaimTransactionModel.builder().accountId(baseKeyPair.getAddress()).name(validName)
                .nameSalt(salt).nonce(getNextBaseKeypairNonce()).ttl(ZERO).build();

        _logger.info(this.aeternityServiceNative.transactions
            .blockingCreateUnsignedTransaction(nameClaimTx).getResult());

        result = this.blockingPostTx(nameClaimTx);
        _logger.info(String.format("Using namespace %s and salt %s for committmentId %s", validName,
            salt, EncodingUtils.generateCommitmentHash(validName, salt)));
  ...
  public void postUpdateAndSpendTxTest(TestContext context) {
    this.executeTest(context, t -> {
      try {
        ...
        NamePreclaimTransactionModel namePreclaimTx =
            NamePreclaimTransactionModel.builder().accountId(baseKeyPair.getAddress()).name(name)
                .salt(salt).nonce(getNextBaseKeypairNonce()).ttl(ZERO).build();
        ...
        NameClaimTransactionModel nameClaimTx =
            NameClaimTransactionModel.builder().accountId(baseKeyPair.getAddress()).name(name)
                .nameSalt(salt).nonce(getNextBaseKeypairNonce()).ttl(ZERO).build();
        PostTransactionResult nameClaimResult = this.blockingPostTx(nameClaimTx);
        _logger.info(String.format("Using namespace %s and salt %s for committmentId %s", name,
            salt, EncodingUtils.generateCommitmentHash(name, salt)));
        _logger.info("NameClaimTx hash: " + nameClaimResult.getTxHash());

        NameEntryResult nameEntryResult = this.aeternityServiceNative.names.blockingGetNameId(name);
        BigInteger initialTTL = nameEntryResult.getTtl();

        _logger.info(String.format(
            "Created namespace %s with salt %s and nameEntry %s in tx %s for update test", name,
            salt, nameEntryResult, nameClaimResult.getTxHash()));
        ...
        NameUpdateTransactionModel nameUpdateTx =
            NameUpdateTransactionModel.builder().accountId(baseKeyPair.getAddress())
                .nameId(nameEntryResult.getId()).nonce(getNextBaseKeypairNonce()).ttl(ZERO)
                .clientTtl(clientTtl).nameTtl(nameTtl).pointers(new HashMap<String, String>() {
                  {
        ...
        _logger.info(String.format(
            "Updated namespace %s with salt %s and nameEntry %s in tx %s for update test", name,
            salt, nameEntryResult, nameUpdateResult.getTxHash()));
        ...
        if (diffTtl < 170000) {
          _logger.info(String.format(
              "Diff of Ttl is %s, this happens when meanwhile new blocks are mined", diffTtl));
        }
        BigInteger aettos = new BigInteger("1000000000000000000");
        SpendTransactionModel spendTx = SpendTransactionModel.builder()
            .sender(this.baseKeyPair.getAddress()).recipient(nameEntryResult.getId()).amount(aettos)
            .payload("send to AENS name test").ttl(ZERO).nonce(getNextBaseKeypairNonce()).build();
    ...
    this.executeTest(context, t -> {
      try {
        ...
        NameRevokeTransactionModel nameRevokeTx =
            NameRevokeTransactionModel.builder().accountId(baseKeyPair.getAddress()).nameId(nameId)
                .nonce(getNextBaseKeypairNonce()).ttl(ZERO).build();
  ...
  public void zAuctionTest(TestContext context) {
    this.executeTest(context, t -> {
      try {
        ...
        _logger.info("active auctions count: {}", oldNameAuctionsResult.getNameAuctions().size());

        /** create a new namespace to update later */
        NamePreclaimTransactionModel namePreclaimTx =
            NamePreclaimTransactionModel.builder().accountId(this.baseKeyPair.getAddress())
                .name(name).salt(salt).nonce(getNextBaseKeypairNonce()).ttl(ZERO).build();
        PostTransactionResult namePreclaimResult = this.blockingPostTx(namePreclaimTx);
        _logger.info("NamePreclaimTx hash: {}", namePreclaimResult.getTxHash());
        context.assertEquals(namePreclaimResult.getTxHash(),
            this.aeternityServiceNative.transactions.computeTxHash(namePreclaimTx));
        ...
        context.assertFalse(this.aeternityServiceNative.indaex.blockingGetNameAuction(name)
            .getAeAPIErrorMessage().isEmpty());

        NameClaimTransactionModel nameClaimTx =
            NameClaimTransactionModel.builder().accountId(this.baseKeyPair.getAddress()).name(name)
                .nameSalt(salt).nonce(getNextBaseKeypairNonce()).ttl(ZERO).build();
        BigInteger currentNameFee = nameClaimTx.getNameFee();
        _logger.info("current nameFee: {} ættos", currentNameFee);
        _logger.info("current nameFee: {} Æ",
            UnitConversionUtil.fromAettos(currentNameFee.toString(), UnitConversionUtil.Unit.AE));
        PostTransactionResult nameClaimResult = this.blockingPostTx(nameClaimTx);
        _logger.info(String.format("Using namespace %s and salt %s for committmentId %s", name,
            salt, EncodingUtils.generateCommitmentHash(name, salt)));
        _logger.info("NameClaimTx hash: {}", nameClaimResult.getTxHash());

        while (this.aeternityServiceNative.indaex.blockingGetNameAuction(name)
            .getAeAPIErrorMessage() != null) {
        ...
        context.assertEquals(oldNameAuctionsResult.getNameAuctions().size() + 1,
        ...
        _logger.info("Created namespace {} with salt {} and nameEntry {} in tx {} for update test",
        ...
        SpendTransactionModel spendTx =
            SpendTransactionModel.builder().sender(account.getPublicKey()).recipient(recipient)
                .amount(amount).ttl(ZERO).nonce(nonce).build();
        PostTransactionResult txResponse = this.blockingPostTx(spendTx);
        _logger.info("SpendTx hash: " + txResponse.getTxHash());
        context.assertEquals(txResponse.getTxHash(),
        ...
        NameClaimTransactionModel nextNameClaimTx = nameClaimTx.toBuilder().accountId(recipient)
            .nonce(otherAccount.getNonce().add(BigInteger.ONE)).nameFee(nextNameFee)
            .nameSalt(BigInteger.ZERO).build();
