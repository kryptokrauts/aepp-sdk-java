    ...
    this.executeTest(context, t -> {
      try {
        ...
        SpendTransactionModel spendTx = SpendTransactionModel.builder()
            .sender(initiator.getAddress()).recipient(responder.getAddress()).amount(amount)
            .payload("").ttl(ZERO).nonce(getNextBaseKeypairNonce()).build();
  ...
  public void channelCreateTest(TestContext context) throws TransactionCreateException {
    this.executeTest(context, t -> {
      ...
      ChannelCreateTransactionModel model = ChannelCreateTransactionModel.builder()
          .initiator(initiator.getAddress()).initiatorAmount(amount)
          .responder(responder.getAddress()).responderAmount(amount).channelReserve(ZERO)
          .lockPeriod(ZERO).ttl(ZERO).stateHash("").nonce(getNextBaseKeypairNonce()).build();
      String txResponse =
          aeternityServiceNative.transactions.blockingCreateUnsignedTransaction(model).getResult();
      context.assertNotNull(txResponse);
