...
public final class KeyPairServiceImpl implements KeyPairService {
	...
	@Override
	public MnemonicKeyPair generateMasterMnemonicKeyPair(String mnemonicSeedPassword) throws AException {
		try {
			...
			throw new AException(String.format("An error occured generating keyPair %s", e.getLocalizedMessage()), e);
		}
	}

	@Override
	public MnemonicKeyPair recoverMasterMnemonicKeyPair(List<String> mnemonicSeedWords, String mnemonicSeedPassword)
			throws AException {
		...
		return new MnemonicKeyPair(generatedKeyPair, mnemonicSeedWords, new DeterministicHierarchy(master));
	}

	@Override
	public MnemonicKeyPair generateDerivedKey(MnemonicKeyPair mnemonicKeyPair, boolean hardened,
			ChildNumber... derivationPath) throws AException {
		...
		/**
		 * following the BIP32 specification create the following tree purpose -> coin
		 * -> account -> external chain -> child address
		 */

		/**
		 * always set path for purpose {@link BaseConstants.HD_CHAIN_PURPOSE}, coin
		 * {@link BaseConstants.HD_CHAIN_CODE_AETERNITY}
		 */
		List<ChildNumber> pathToDerivedKey = new LinkedList<ChildNumber>();
		pathToDerivedKey.addAll(Arrays.asList(new ChildNumber(BaseConstants.HD_CHAIN_PURPOSE, true),
		...
		if (derivationPath == null || derivationPath.length == 0) {
			...
			_logger.warn(
					String.format("You are using a custom key derivation path - this will be appended to m/%sh/%sh",
							BaseConstants.HD_CHAIN_PURPOSE, BaseConstants.HD_CHAIN_CODE_AETERNITY));
			pathToDerivedKey.addAll(Arrays.asList(derivationPath));
		}

		DeterministicKey nextChildDeterministicKey = mnemonicKeyPair.getDeterministicHierarchy()
				.deriveNextChild(ImmutableList.copyOf(pathToDerivedKey), false, true, hardened);

		// derive a new child
		RawKeyPair childRawKeyPair = generateRawKeyPairFromSecret(nextChildDeterministicKey.getPrivateKeyAsHex());

		return new MnemonicKeyPair(childRawKeyPair, mnemonicKeyPair.getMnemonicSeedWords(),
