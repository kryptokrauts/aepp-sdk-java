...
public class InfoServiceImpl implements InfoService {
	...
	public Single<TransactionResult> asyncGetTransactionByHash(String txHash) {
		return TransactionResult.builder().build()
				.asyncGet(externalApi.rxGetTransactionByHash(txHash));
	}
	...
	@Override
	public Single<TransactionInfoResult> asyncGetTransactionInfoByHash(
			String txHash) {
		return TransactionInfoResult.builder().build()
				.asyncGet(externalApi.rxGetTransactionInfoByHash(txHash));
	}

	@Override
	public TransactionInfoResult blockingGetTransactionInfoByHash(
			String txHash) {
		return TransactionInfoResult.builder().build()
	...
	@Override
	public Single<TransactionResults> asyncGetMicroBlockTransactions(
			final String microBlockHash) {
		this.validateMicroTxHash(microBlockHash);
		return TransactionResults.builder().build().asyncGet(this.externalApi
				.rxGetMicroBlockTransactionsByHash(microBlockHash));
	}

	@Override
	public TransactionResults blockingGetMicroBlockTransactions(
			final String microBlockHash) {
		this.validateMicroTxHash(microBlockHash);
		return TransactionResults.builder().build().blockingGet(this.externalApi
				.rxGetMicroBlockTransactionsByHash(microBlockHash));
	}

	@Override
	public StringResultWrapper blockingGetContractByteCode(
			final String contractId) {
		this.validateContractId(contractId);
		return StringResultWrapper.builder().build()
				.blockingGet(this.externalApi.rxGetContractCode(contractId)
						.map(contactCode -> contactCode.getBytecode()));
	}

	@Override
	public Single<StringResultWrapper> asnycGetContractByteCode(
			final String contractId) {
		this.validateContractId(contractId);
		return StringResultWrapper.builder().build()
				.asyncGet(this.externalApi.rxGetContractCode(contractId)
						.map(contactCode -> contactCode.getBytecode()));
	}

	private void validateMicroTxHash(final String microBlockHash) {
		ValidationUtil.checkParameters(
				validate -> Optional.ofNullable(microBlockHash
						.startsWith(ApiIdentifiers.MICRO_BLOCK_HASH)),
				microBlockHash, "getMicroBlockTransactions",
	...
	private void validateContractId(final String contractId) {
		ValidationUtil.checkParameters(
				validate -> Optional.ofNullable(
						contractId.startsWith(ApiIdentifiers.CONTRACT_PUBKEY)),
	...
	public Single<KeyBlockResult> asyncGetCurrentKeyBlock() {
		return KeyBlockResult.builder().build()
				.asyncGet(this.externalApi.rxGetCurrentKeyBlock());
	}

	@Override
	public KeyBlockResult blockingGetCurrentKeyBlock() {
		return KeyBlockResult.builder().build()
				.blockingGet(this.externalApi.rxGetCurrentKeyBlock());
	}
}
