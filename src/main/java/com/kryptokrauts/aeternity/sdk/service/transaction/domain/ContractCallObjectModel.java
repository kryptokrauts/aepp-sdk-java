...
public class ContractCallObjectModel extends GenericResultObject<ContractCallObject, ContractCallObjectModel> {
	...
	@Default
	private List<EventModel> log = new ArrayList<>();
	...
	public ContractCallObjectModel map(ContractCallObject generatedResultObject) {
		if (generatedResultObject != null)
			return this.toBuilder().callerId(generatedResultObject.getCallerId())
					.callerNonce(generatedResultObject.getCallerNonce()).height(generatedResultObject.getHeight())
					.contractId(generatedResultObject.getContractId()).gasPrice(generatedResultObject.getGasPrice())
					.log(generatedResultObject.getLog().stream().map(event -> EventModel.builder().build().map(event))
							.collect(Collectors.toList()))
					.returnType(generatedResultObject.getReturnValue())
					.returnValue(generatedResultObject.getReturnValue()).gasUsed(generatedResultObject.getGasUsed())
					.build();
		else
