...
public final class AccountServiceImpl implements AccountService {
  ...
  public AccountResult blockingGetAccount(final Optional<String> base58PublicKey) {
    return AccountResult.builder().build().blockingGet(externalApi.rxGetAccountByPubkey(
        base58PublicKey.orElseGet(() -> config.getBaseAccount().getAddress())));
  }

  @Override
  public Single<AccountResult> asyncGetAccount(final Optional<String> base58PublicKey) {
    return AccountResult.builder().build().asyncGet(externalApi.rxGetAccountByPubkey(
        base58PublicKey.orElseGet(() -> config.getBaseAccount().getAddress())));
  }
