...
@ToString
public class TransactionInfoResult extends GenericResultObject<TxInfoObject, TransactionInfoResult> {
	...
	protected TransactionInfoResult map(TxInfoObject generatedResultObject) {
		if (generatedResultObject != null)
			return this.toBuilder().txInfo(generatedResultObject.getTxInfo())
					.callInfo(ContractCallObjectModel.builder().build().map(generatedResultObject.getCallInfo()))
					.gaInfo(GAObjectModel.builder().build().map(generatedResultObject.getGaInfo())).build();
		else
