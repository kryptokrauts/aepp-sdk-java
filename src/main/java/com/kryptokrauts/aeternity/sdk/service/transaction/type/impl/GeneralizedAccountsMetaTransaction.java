...
public class GeneralizedAccountsMetaTransaction
  ...
  public Bytes createRLPEncodedList() {
    Bytes encodedRlp = RLP.encodeList(rlpWriter -> {
      rlpWriter.writeInt(SerializationTags.OBJECT_TAG_GENERALIZED_ACCOUNTS_META_TRANSACTION);
      rlpWriter.writeInt(SerializationTags.VSN_2);
      byte[] gaIdWithTag = EncodingUtils.decodeCheckAndTag(model.getGaId(),
          Arrays.asList(ApiIdentifiers.ACCOUNT_PUBKEY));
      rlpWriter.writeByteArray(gaIdWithTag);
      rlpWriter.writeByteArray(EncodingUtils.decodeCheckWithIdentifier(model.getAuthData()));
