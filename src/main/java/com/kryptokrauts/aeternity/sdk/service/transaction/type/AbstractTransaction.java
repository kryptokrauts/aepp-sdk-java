...
public abstract class AbstractTransaction<TxModel> {
	...
	/**
	 * this method creates an unsigned transaction whether in native or debug mode
	 * if no fee is defined (null), the fee will be calculated based on the
	 * transactions byte size and maybe other transaction attributes using the fee
	 * calculation model
	 *
	 * @param nativeMode      native or debug mode
	 * @param minimalGasPrice the minimal gas price, which the fee is multiplied
	 *                        with *
	 * @return a single-wrapped unsignedTx object
	 */
	public Single<UnsignedTx> createUnsignedTransaction(boolean nativeMode, long minimalGasPrice) {
		if (nativeMode) {
			...
			if (fee == null) {
				...
				_logger.debug(String.format("Using calculation model %s the following fee was calculated %s",
			...
			return Single.just(new UnsignedTx()
					.tx(EncodingUtils.encodeCheck(encodedRLPArrayWithFee.toArray(), ApiIdentifiers.TRANSACTION)));
		}
	...
	/**
	 * @return the transaction model which is actually used to create the remote
	 *         call to the generated api functions (classes of package
	 *         {@link com.kryptokrauts.aeternity.generated.api.rxjava})
	 */
