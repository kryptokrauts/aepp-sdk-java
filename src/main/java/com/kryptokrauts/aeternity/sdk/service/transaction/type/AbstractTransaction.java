...
/**
 * this abstract class is the base of all transactions and wraps the calculation
 * of transaction fees by using a default fee, generating the RLP and gaining
 * the byte size of the transaction
 *
 * @param <TxModel>
 */
@SuperBuilder
public abstract class AbstractTransaction<TxModel extends AbstractTransactionModel<?>> {
	 ...
	 * calculating the fee based on RLP encoding
	 *
	 * @return {@link Bytes}
	 */
	...
	/**
	 * this method needs to be implemented for testing purposes (non native
	 * mode)
	 *
	...
	/**
	 * this method creates an unsigned transaction whether in native or debug
	 * mode if no fee is defined (null), the fee will be calculated based on the
	 * transactions byte size and maybe other transaction attributes using the
	 * fee calculation model
	 *
	 * @param nativeMode
	 *            native or debug mode
	 * @param minimalGasPrice
	 *            the minimal gas price, which the fee is multiplied with *
	 * @return a single-wrapped unsignedTx object
	 */
	public Single<UnsignedTx> createUnsignedTransaction(boolean nativeMode,
			long minimalGasPrice) {
		/** before creating the unsigned transaction we validate the input */
		model.checkMandatoryFields();
		model.validateInput();
		if (nativeMode) {
			/**
			 * if no fee is given - use default fee to create a tx and get its
			 * size
			 */
			if (model.getFee() == null) {
				...
				model.setFee(getFeeCalculationModel().calculateFee(byte_size,
						minimalGasPrice, this));
				_logger.info(String.format(
						"Using calculation model %s the following fee was calculated %s",
						getFeeCalculationModel().getClass().getName(),
						model.getFee()));
			} else {
				_logger.warn(
			...
			return Single.just(new UnsignedTx().tx(
					EncodingUtils.encodeCheck(encodedRLPArrayWithFee.toArray(),
							ApiIdentifiers.TRANSACTION)));
		}
		return createInternal();
	}

	/**
	 * check if value is 0 -> if so, serialize as byte, otherwise serialize
	 * value as BigInteger
	 *
	 * @param rlpWriter
	 * @param value
	 */
	protected void checkZeroAndWriteValue(RLPWriter rlpWriter,
			BigInteger value) {
