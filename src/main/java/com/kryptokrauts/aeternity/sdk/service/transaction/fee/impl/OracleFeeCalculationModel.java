...
public class OracleFeeCalculationModel implements FeeCalculationModel {
	 ...
	 *
	 * <p>
	 * ceiling(32000 * RelativeTTL / floor(60 * 24 * 365 / key_block_interval))+
	 * byte_size(OracleRegisterTx) * GasPerByte
	 */
	@Override
	public BigInteger calculateFee(int tx_byte_size, long minimalGasPrice, AbstractTransaction<?> transaction) {
		long relativeTtl;
		if (transaction instanceof OracleRegisterTransaction) {
			relativeTtl = ((OracleRegisterTransaction) transaction).getModel().getOracleTtl().getValue().longValue();
		} else if (transaction instanceof OracleExtendTransaction) {
			relativeTtl = ((OracleExtendTransaction) transaction).getModel().getOracleRelativeTtl().getValue()
					.longValue();
		} else if (transaction instanceof OracleQueryTransaction) {
			relativeTtl = ((OracleQueryTransaction) transaction).getModel().getQueryTtl().getValue().longValue();
		} else if (transaction instanceof OracleRespondTransaction) {
			relativeTtl = ((OracleRespondTransaction) transaction).getModel().getResponseTtl().getValue().longValue();
		} else {
			throw new UnsupportedOperationException(
					String.format("Fee calculation not supported for class %s", transaction.getClass().getName()));
		}
		long fee = ((BaseConstants.BASE_GAS + tx_byte_size * BaseConstants.GAS_PER_BYTE)
				+ (long) Math.ceil(32000 * relativeTtl / Math.floor(60 * 24 * 365 / BaseConstants.KEY_BLOCK_INTERVAL)))
				* minimalGasPrice;
