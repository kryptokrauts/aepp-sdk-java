...
public class TransactionFactory {
	...
	public SpendTransaction createSpendTransaction(String sender, String recipient, BigInteger amount, String payload,
			BigInteger ttl, BigInteger nonce) {
	...
	public CreateChannelDepositTransaction createChannelDepositTransaction(String channelId, String fromId,
			BigInteger amount, BigInteger ttl, String stateHash, BigInteger round, BigInteger nonce) {
		return CreateChannelDepositTransaction.builder().channelId(channelId).fromId(fromId).amount(amount).ttl(ttl)
	...
	public StaticContractCallTransaction createStaticContractCallTransaction(BigInteger abiVersion, String callData,
			String contractId, BigInteger gas, BigInteger nonce, String callerId, BigInteger ttl) {
		return StaticContractCallTransaction.builder().abiVersion(abiVersion).amount(BigInteger.ZERO) // we provide a
																										// separate
				// setter for the
				// optional amount
				.callData(callData).callerId(callerId).contractId(contractId).gas(gas)
				.gasPrice(BigInteger.valueOf(BaseConstants.MINIMAL_GAS_PRICE)).nonce(nonce).ttl(ttl)
				.contractApi(contractApi).debugApi(debugApi).feeCalculationModel(contractCallFeeCalculationModel)
				.build();
	}
}
