...
public class TransactionFactory {
	...
	private static FeeCalculationModel contractFeeCalculationModel = new ContractFeeCalculationModel();
	 ...
	 * @param fee       fee, if null the minimal transaction fee will be
	 *                  automatically calculated (default)
	 * @param ttl       time to live
	 * @param nonce     account nonce
	 * @return a {@link SpendTransaction} object
	 */
	public SpendTransaction createSpendTransaction(String sender, String recipient, BigInteger amount, String payload,
			@Nullable BigInteger fee, BigInteger ttl, BigInteger nonce) {
		return SpendTransaction.builder().sender(sender).recipient(recipient).amount(amount).payload(payload).fee(fee)
				.ttl(ttl).nonce(nonce).feeCalculationModel(baseFeeCalculationModel).transactionApi(transactionApi)
				.build();
	}
	 ...
	 * @return
	 */
	public CreateChannelDepositTransaction createChannelDepositTransaction(String channelId, String fromId,
			BigInteger amount, BigInteger fee, BigInteger ttl, String stateHash, BigInteger round, BigInteger nonce) {
		return CreateChannelDepositTransaction.builder().channelId(channelId).fromId(fromId).amount(amount).fee(fee)
				.ttl(ttl).stateHash(stateHash).round(round).nonce(nonce).feeCalculationModel(baseFeeCalculationModel)
	...
	public CreateContractTransaction createContractCreateTransaction(BigInteger abiVersion, BigInteger amount,
		...
		return CreateContractTransaction.builder().abiVersion(abiVersion).amount(amount).callData(callData)
				...
				.ownerId(ownerId).ttl(ttl).vmVersion(vmVersion).feeCalculationModel(contractFeeCalculationModel)
	...
	public CreateContractTransaction createContractCreateTransaction(BigInteger abiVersion, BigInteger amount,
		...
		return CreateContractTransaction.builder().abiVersion(abiVersion).amount(amount).callData(callData)
				.contractByteCode(contractByteCode).deposit(deposit).gas(gas).gasPrice(gasPrice).nonce(nonce)
				.ownerId(ownerId).ttl(ttl).vmVersion(vmVersion).feeCalculationModel(contractFeeCalculationModel)
				.fee(fee).contractApi(contractApi).compilerApi(compilerApi).build();
	}
}
