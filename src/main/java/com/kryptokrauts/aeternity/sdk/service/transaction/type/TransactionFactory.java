...
public class TransactionFactory {
	...
	public ContractCallTransaction createContractCallTransaction(BigInteger abiVersion, BigInteger amount,
			String callData, String contractId, BigInteger gas, BigInteger gasPrice, BigInteger nonce, String callerId,
			BigInteger ttl) {
		return ContractCallTransaction.builder().abiVersion(abiVersion).amount(amount).callData(callData)
				.callerId(callerId).contractId(contractId).gas(gas).gasPrice(gasPrice).nonce(nonce).ttl(ttl)
				.contractApi(contractApi).feeCalculationModel(contractCallFeeCalculationModel).build();
	}
}
