...
public class WalletServiceImpl implements WalletService {
    ...
    public String generateKeystore( RawKeyPair rawKeyPair, String walletPassword, String walletName ) throws AException {
        ...
        byte[] rawHash = argon2Advanced
        .rawHash( config.getOpsLimit(), config.getMemlimitKIB(), config.getParallelism(), walletPassword.toCharArray(), StandardCharsets.UTF_8, salt );
        ...
        .crypto( Keystore.Crypto.builder().secretType( config.getSecretType() ).symmetricAlgorithm( config.getSymmetricAlgorithm() )
        ...
        .kdfParams( Keystore.KdfParams.builder().memLimitKib( config.getMemlimitKIB() ).opsLimit( config.getOpsLimit() ).salt( Hex.toHexString( salt ) )
        .parallelism( config.getParallelism() ).build() ).build() ).id( UUID.randomUUID().toString() ).name( walletName ).version( config.getVersion() )
        .build();
