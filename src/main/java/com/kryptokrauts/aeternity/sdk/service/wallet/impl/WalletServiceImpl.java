...
public class WalletServiceImpl implements WalletService {
  ...
  @Nonnull private WalletServiceConfiguration config;
  ...
  @Override
  public String generateKeystore(RawKeyPair rawKeyPair, String walletPassword, String walletName)
      throws AException {
    ...
    byte[] privateAndPublicKey =
        new byte[rawKeyPair.getPrivateKey().length + rawKeyPair.getPublicKey().length];
    System.arraycopy(
        rawKeyPair.getPrivateKey(), 0, privateAndPublicKey, 0, rawKeyPair.getPrivateKey().length);
    System.arraycopy(
        rawKeyPair.getPublicKey(),
        0,
        privateAndPublicKey,
        rawKeyPair.getPrivateKey().length,
    ...
    byte[] ciphertext =
        SecretBox.encrypt(privateAndPublicKey, SecretBox.Key.fromBytes(rawHash), nonce);
    ...
    Keystore wallet =
        Keystore.builder()
            .publicKey(getPublicKey(rawKeyPair))
            .crypto(
  ...
  @Override
  public byte[] recoverPrivateKeyFromKeystore(String json, String walletPassword)
      throws AException {
    try {
